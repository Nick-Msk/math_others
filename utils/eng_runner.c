...
int                             eng_fautoprint(FILE *f, struct eng_int_interval v){
	...
	fprintf(f, "%*cfromX=%d, toX=%d, fromY=%d, toY=%d, stopRun=%d, printFlag=%d, modLog=%d f_int1dim=%p f_int2dim=%p\n",
		logoffset, '|', v.fromX, v.toX, v.fromY, v.toY, v.stopRun, v.printFlag, v.modLog, v.f_int_1dim, v.f_int_2dim);	// logoffset must return 0 if log isn't enabled
	return cnt;
}
